...
inline uint8_t get_piece_type_of_field(const C_BoardState &board_state, int position_idx)
{
    uint8_t piece = 0;
    uint64_t mask = most_sig_bit >> position_idx;
    piece += pKing * (bool)(mask & board_state.pieces[b_kings]);
    piece += pQueen * (bool)(mask & board_state.pieces[b_queens]);
    piece += pBishop * (bool)(mask & board_state.pieces[b_bishops]);
    piece += pKnight * (bool)(mask & board_state.pieces[b_knights]);
    piece += pRook * (bool)(mask & board_state.pieces[b_rooks]);
    piece += pPawn * (bool)(mask & board_state.pieces[b_pawns]);
    return piece;
}
...
inline std::string move_to_uci_str(const move m)
{
    if (m.promotion != 0)
    {
        return field_id_to_string(m.src) + field_id_to_string(m.dest) + piece_code_to_char.at(m.promotion);
    }
    else
    {
